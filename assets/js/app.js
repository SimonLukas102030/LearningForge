...
async function renderTopic(subjectId, yearId, topicId) {
  ...
  // Phase-0 Plugin-System: Legacy-Wrap-Pfad (meta.content ohne subtopics) hat
  // alle Widget-Slots direkt im topicBody. Multi-subtopic-Pfad mountet erst
  // beim openSubtopic. mountAllWidgets ist idempotent + ignoriert leere
  // Container, daher harmlos auch wenn keine Slots da sind.
  const topicBody = document.getElementById('topicBody');
  if (topicBody) {
    mountAllWidgets(topicBody).catch(err => console.error('[LF] widget mount failed:', err));
  }
...
function renderBlock(block) {
  ...
  if (type === 'widget') {
    ...
    // Phase-0 Plugin-System: bekannte Widget-Typen werden als Slot-Stubs gerendert.
...
Object.assign(window.LF, {
  ...
  openSubtopic: (idx) => {
    ...
    // Phase-0 Plugin-System: alle [data-lf-widget]-Slots lazy-mounten. Async
    // im Hintergrund — User sieht erst Skeleton, dann Widget. Errors fangen
    // wir ab, damit ein crashendes Widget die Subtopic-Page nicht killt.
    mountAllWidgets(grid).catch(err => console.error('[LF] widget mount failed:', err));
  ...
  closeSubtopic: () => {
    ...
    const grid = document.getElementById('subtopicGrid');
    if (!grid) return;
    // Phase-0 Plugin-System: VOR innerHTML-Reset alle Widget-Instances cleanen
    // (RAF-Loops stoppen, Listener entfernen). Sonst leakt der Sub-Tree.
    unmountAllIn(grid);
